...
function InviteForm() {
  ...
  return <form className="form-stack" onSubmit={submit}><label>Seu nome<input value={nome} onChange={e=>setNome(e.target.value)} minLength={2} maxLength={150} autoComplete="name" required/></label><label>Crie uma senha<input type="password" value={senha} onChange={e=>setSenha(e.target.value)} minLength={12} maxLength={128} autoComplete="new-password" required/></label><label>Confirme a senha<input type="password" value={confirmacao} onChange={e=>setConfirmacao(e.target.value)} minLength={12} maxLength={128} autoComplete="new-password" required/></label><small className="muted">Use pelo menos 12 caracteres.</small>{error&&<p className="form-error" role="alert">{error}</p>}<Button className="primary" disabled={loading}>{loading?<Spinner/>:<UserPlus size={16}/>}Aceitar convite</Button></form>;
}
