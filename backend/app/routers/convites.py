...
def criar_convite(
    ...
    db.execute(
        update(Convite)
        .where(Convite.empresa_id == usuario.empresa_id, Convite.email == email, Convite.status == "pendente")
    ...
    convite = Convite(
        ...
        expira_em=agora + timedelta(days=settings.INVITE_EXPIRE_DAYS),
    )
    db.add(convite)
    db.commit()
    db.refresh(convite)
...
def cancelar_convite(
    convite_id: str, usuario: Usuario = Depends(require_manager), db: Session = Depends(get_db)
) -> Convite:
...
def reenviar_convite(
    ...
    convite = db.scalar(
        select(Convite).where(
            Convite.id == convite_id, Convite.empresa_id == usuario.empresa_id, Convite.status == "pendente"
        )
    )
    if not convite:
        raise HTTPException(status_code=404, detail="Convite pendente não encontrado.")
    ...
    convite.expira_em = datetime.now(UTC) + timedelta(days=settings.INVITE_EXPIRE_DAYS)
    db.commit()
    db.refresh(convite)
...
def aceitar_convite(dados: ConviteAceitar, db: Session = Depends(get_db)) -> Usuario:
    convite = db.scalar(
        select(Convite).where(
            Convite.token_hash == sha256(dados.token.encode()).hexdigest(), Convite.status == "pendente"
        )
    )
    if not convite or convite.expira_em < datetime.now(UTC):
    ...
    novo_usuario = usuario_service.criar_usuario(
        db, UsuarioCreate(nome=dados.nome, email=convite.email, senha=dados.senha, empresa_id=convite.empresa_id)
    )
